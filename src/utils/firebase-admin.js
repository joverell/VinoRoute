...
const initializeFirebaseAdmin = () => {
    if (admin.apps.length === 0) {
        ...
        if (!projectId || !clientEmail || !privateKey || !storageBucket) {
            throw new FirebaseAdminInitializationError('Firebase Admin SDK credentials are not set in the environment. Please ensure FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY and FIREBASE_STORAGE_BUCKET are set.');
        }
        try {
            admin.initializeApp({
